...
class SphereBoxDetection
    : public ::testing::TestWithParam<SphereBoxDetectionCase> {};

TEST_P(SphereBoxDetection, _) {
  const auto &c = GetParam();

  engine::physics::Sphere sphere(c.sphere_radius, 1.0f, c.sphere_pos,
                                 glm::vec3(0.0f));
  ...
  std::string msg =
      "Failure for Sphere v. Box collision detection. TEST CASE -> " + c.label;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        SphereBoxDetectionCase{
            .sphere_pos = glm::vec3(0.5f + 1.0f - 0.01f,
                                    0.5f + std::sqrt(3.0f) - 0.01f, 0.0f),
            .sphere_radius = 2.0f,
            .box_rotation = glm::quat(),
            .expect_collision = true,
        ...
        SphereBoxDetectionCase{
            .sphere_pos = glm::vec3(
                (std::sqrt(2 * (0.5f * 0.5f))) + 1.0f - 0.01f, 0.0f, 0.0f),
            .sphere_radius = 1.0f,
            .box_rotation = glm::quat(glm::angleAxis(glm::radians(45.0f),
                                                     glm::vec3(0, 1, 0))),
            .expect_collision = true,
            .label = "TouchingDueToRotation_DoesCollide"}),
    [](const testing::TestParamInfo<SphereBoxDetectionCase> &info) {
...
TEST(SphereBoxRotation, HeavierBox_RotatesLess) {
  ...
  auto runCollision = [](float box_mass) -> std::pair<bool, float> {
    ...
    bool collided =
        engine::physics::Collisions::computeContact(box, sphere, contact);
    if (collided) {
      engine::physics::Collisions::resolveElasticCollision(box, sphere,
                                                           contact);
    }
...
TEST(SphereBoxRotation, HeavierSphere_ImpartsMoreRotation) {
  auto runCollision = [](float sphere_mass) -> std::pair<bool, float> {
    engine::physics::Sphere sphere(1.0f, sphere_mass,
                                   glm::vec3(1.499f, 0.3f, 0.0f),
                                   glm::vec3(-2.0f, 0.0f, 0.0f));
    engine::physics::Box box(glm::vec3(1.0f), glm::vec3(0.0f), glm::vec3(0.0f),
                             5.0f, glm::quat());
...
TEST(SphereBoxRotation, LargerLeverArm_CausesMoreRotation) {
  auto runCollision = [](float y_offset) -> std::pair<bool, float> {
    engine::physics::Sphere sphere(1.0f, 1.0f,
                                   glm::vec3(1.499f, y_offset, 0.0f),
                                   glm::vec3(-2.0f, 0.0f, 0.0f));
    engine::physics::Box box(glm::vec3(1.0f), glm::vec3(0.0f), glm::vec3(0.0f),
                             5.0f, glm::quat());
...
TEST(SphereBoxRotation, TangentialImpact_CausesMoreRotation) {
  ...
  // different angle)
  auto [tangential_collided, tangential_rotation] =
      runCollision(glm::vec3(-2.0f, 0.0f, 0.0f));

  // Direct head-on: sphere moving straight toward contact point
  auto [direct_collided, direct_rotation] =
      runCollision(glm::vec3(-1.4f, -1.4f, 0.0f));

  ASSERT_TRUE(tangential_collided)
      << "Tangential impact collision not detected";
  ...
  EXPECT_GT(tangential_rotation, 1.2f * direct_rotation)
      << "More tangential impact should cause more rotation "
      << "(expected >1.2x, got " << tangential_rotation / direct_rotation
      << "x)";
}
...
TEST(SphereBoxResolution, ZeroRelativeVelocity_OnlyAdjustsPosition) {
  // Objects touching with matched velocities - should have no collision
  // response
  engine::physics::Sphere sphere(1.0f, 1.0f, glm::vec3(1.4f, 0.0f, 0.0f),
                                 glm::vec3(-1.0f, 0.0f, 0.0f));
  engine::physics::Box box(glm::vec3(1.0f), glm::vec3(0.0f),
                           glm::vec3(-1.0f, 0.0f, 0.0f), 5.0f, glm::quat());
  ...
  // resolution)
  test_util::ExpectVec3Near(sphere_vel_before, sphere.velocity,
                            "Sphere velocity", 1e-6f);
  test_util::ExpectVec3Near(box_vel_before, box.velocity, "Box velocity",
                            1e-6f);
