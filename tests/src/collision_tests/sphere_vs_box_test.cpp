...
TEST(SphereBoxRotation, LargerLeverArm_CausesMoreRotation) {
  ...
  EXPECT_GT(far_from_center, 3.0f * near_center)
      << "Impact at 0.4 should cause significantly more rotation than at 0.1 " << "(expected ~4x, got "
      << far_from_center / near_center << "x)";
}
...
TEST(SphereBoxResolution, ZeroRelativeVelocity_OnlyAdjustsPosition) {
  ...
  // response
  engine::physics::Sphere sphere(1.0f, 1.0f, glm::vec3(1.4f, 0.0f, 0.0f), glm::vec3(-1.0f, 0.0f, 0.0f));
  engine::physics::Box box(glm::vec3(1.0f), glm::vec3(0.0f), glm::vec3(-1.0f, 0.0f, 0.0f), 5.0f, glm::quat());
...
TEST(SphereBoxImmovable, SphereBouncesOffImmovableBox) {
  engine::physics::Sphere sphere(1.0f, 1.0f, glm::vec3(1.499f, 0.0f, 0.0f), glm::vec3(-2.0f, 0.0f, 0.0f));
  engine::physics::Box box(glm::vec3(1.0f), glm::vec3(0.0f), glm::vec3(0.0f), 0.0f, glm::quat());
  ...
  EXPECT_EQ(box.angular_velocity, glm::vec3(0.0f)) << "Immovable box should not rotate";
}
...
TEST(SphereBoxImmovable, OffCenterHit_ImmovableBoxUnchanged) {
  ...
  TestUtil::ExpectVec3Near(box_pos_before, box.position, "Immovable box position should not change");
  EXPECT_EQ(box.angular_velocity, glm::vec3(0.0f)) << "Immovable box should not rotate";
  EXPECT_GT(glm::length(sphere.velocity), 0.0f) << "Sphere should be deflected";
}

TEST(SphereBoxImmovable, PenetrationCorrection_OnlySphereMovesOut) {
  engine::physics::Sphere sphere(1.0f, 1.0f, glm::vec3(1.4f, 0.0f, 0.0f), glm::vec3(-2.0f, 0.0f, 0.0f));
  engine::physics::Box box(glm::vec3(1.0f), glm::vec3(0.0f), glm::vec3(0.0f), 0.0f, glm::quat());
  ...
  engine::physics::Contact contact_after;
  EXPECT_FALSE(engine::physics::Collisions::ComputeContact(box, sphere, contact_after))
