...
{
    return glm::quat(
        glm::vec3(glm::radians(euler.x), glm::radians(euler.y), glm::radians(euler.z)));
}
...
TEST_P(BoxBoxDetection, _)
{
    ...
    engine::physics::Box box_a(glm::vec3(1.0f), c.box_a_position, glm::vec3(0.0f), 1.0f,
                               c.box_a_rotation);
    engine::physics::Box box_b(glm::vec3(1.0f), c.box_b_position, glm::vec3(0.0f), 1.0f,
                               c.box_b_rotation);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        BoxBoxDetectionCase{
            ...
            .box_a_rotation   = glm::quat(),
            .box_b_position   = glm::vec3(0.5f + std::sqrt(0.5f) + 0.001f, 0.0f, 0.0f),
            .box_b_rotation   = getRotationFromEulerAngles(glm::vec3(0.0f, 45.0f, 0.0f)),
            .expect_collision = false,
        ...
        BoxBoxDetectionCase{
            .box_a_position   = glm::vec3(0.0f),
            .box_a_rotation   = glm::quat(),
            .box_b_position   = glm::vec3(0.5f + std::sqrt(0.75f) + 0.001f, 0.0f, 0.0f),
        ...
        BoxBoxDetectionCase{
            .box_a_position   = glm::vec3(0.0f),
            .box_a_rotation   = getRotationFromEulerAngles(glm::vec3(0.0f, 45.0f, 0.0f)),
            .box_b_position   = glm::vec3(std::sqrt(0.5f) + std::sqrt(0.75f) - 0.001f, 0.0f, 0.0f),
            .box_b_rotation   = getDiagonalAlignedOrientation({1, 0, 0}),
            .expect_collision = true,
        ...
        BoxBoxDetectionCase{
            ...
            .box_b_position   = glm::vec3(2.0f * std::sqrt(0.75f) - 0.001f, 0.0f, 0.0f),
            .box_b_rotation   = getDiagonalAlignedOrientation({1, 0, 0}),
            .expect_collision = true,
...
TEST_F(BoxBoxDetection, BoxesCollideDueToScale)
{
    ...
    engine::physics::Box box_a(glm::vec3(1.0f), glm::vec3(), glm::vec3(), 1.0f, glm::quat());
    engine::physics::Box box_b(smaller_scale,
                               glm::vec3(0.5f + std::sqrt(0.75f) + 0.001f, 0.0f, 0.0f),
    ...
    EXPECT_TRUE(larger_box_collided)
        << "Boxes not detecting scale-caused collision. (Did not detect enlarged box)";
}
