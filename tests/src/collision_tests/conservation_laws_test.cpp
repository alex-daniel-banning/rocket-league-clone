...
class CollisionResolutionTest : public ::testing::Test {
 protected:
  ...
  glm::vec3 TotalLinearMomentum(const engine::physics::Sphere& s,
                                const engine::physics::Box& b) {
    return s.mass * s.velocity + b.mass * b.velocity;
  }

  // Helper to compute total angular momentum
  glm::vec3 TotalAngularMomentum(const engine::physics::Sphere& s,
                                 const engine::physics::Box& b) {
    // Orbital angular momentum: L_orbital = r x p
    glm::vec3 sphere_orbital_momentum =
        glm::cross(s.position, s.mass * s.velocity);
    glm::vec3 box_orbital_momentum =
        glm::cross(b.position, b.mass * b.velocity);

    // Spin angular momentum: L_spin = I * w
    glm::vec3 sphere_spin_momentum =
        glm::vec3(0.0f);  // Not implemented yet (no friction)
    glm::vec3 box_spin_momentum = b.inertia_tensor * b.angular_velocity;

    return sphere_orbital_momentum + box_orbital_momentum +
           sphere_spin_momentum + box_spin_momentum;
  }

  // Helper to compute total kinetic energy
  float TotalKineticEnergy(const engine::physics::Sphere& s,
                           const engine::physics::Box& b) {
...
class SphereBox : public CollisionResolutionTest,
                  public ::testing::WithParamInterface<std::string> {
 protected:
  // Different collision scenarios
  void SetupScenario(const std::string& scenario,
                     engine::physics::Sphere& sphere,
                     engine::physics::Box& box) {
    if (scenario == "Approaching") {
      sphere =
          engine::physics::Sphere(1.0f, 2.0f, glm::vec3(1.499f, 0.0f, 0.0f),
                                  glm::vec3(-2.0f, 0.0f, 0.0f));
      box =
          engine::physics::Box(glm::vec3(1.0f), glm::vec3(0.0f),
      ...
      sphere =
          engine::physics::Sphere(1.0f, 1.0f, glm::vec3(1.499f, 0.3f, 0.0f),
                                  glm::vec3(-2.0f, 0.0f, 0.0f));
      box = engine::physics::Box(glm::vec3(1.0f), glm::vec3(0.0f),
                                 glm::vec3(0.0f), 5.0f, glm::quat());
    }
  }
};
...
TEST_P(SphereBox, LinearMomentum) {
  ...
  TestUtil::ExpectVec3Near(momentum_before, momentum_after, "Linear momentum",
                           1e-5f);
}
...
TEST_P(SphereBox, AngularMomentum) {
  ...
  TestUtil::ExpectVec3Near(momentum_before, momentum_after, "Angular momentum",
                           1e-5f);
}
...
INSTANTIATE_TEST_SUITE_P(ConservationLaws, SphereBox,
                         ::testing::Values("Approaching", "SameDirection",
                                           "OffCenter"),
