...
int main() {
  ...
  engine::render::Camera camera(glm::vec3(3.0f, 7.0f, 7.01f),
  ...
  engine::render::Model car(
      engine::PathManager::globalAsset("car/car.obj").c_str());
  ...
  engine::physics::Plane ground(10.0f, 10.0f);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    lightProjection =
        glm::ortho(-5.0f, 5.0f, -5.0f, 5.0f, LIGHT_PROJECTION_NEAR_PLANE,
    ...
    projection = glm::perspective(
        camera.projection.fov, screenWidth / screenHeight,
    ...
    modelLoadingShader.setVec3("lightPos", lightPos);
    modelLoadingShader.setVec3("viewPos", camera.Position);
    renderer.drawPhysicsPlane(ground, modelLoadingShader);
