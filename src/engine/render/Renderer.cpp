...
void Renderer::drawPhysicsPlane(const physics::Plane& plane, Shader& shader,
  ...
  modelMatrix = glm::scale(
      modelMatrix, glm::vec3(plane.getXLength(), 1.0f, plane.getZLength()));
  shader.setMat4("model", modelMatrix);
  shader.setMat4("view", view);
  shader.setMat4("projection", projection);
...
void Renderer::initLine() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(lineVertices), lineVertices,
               GL_STATIC_DRAW);
...
void Renderer::initWireCube() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(cubeWireVertices), cubeWireVertices,
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(cubeWireIndices),
               cubeWireIndices, GL_STATIC_DRAW);
