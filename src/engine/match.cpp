...
void Match::Tick(float delta_time) {
  ...
  while (accumulator_ >= fixed_dt) {
    ...
    contact_constraint_solver_.PreSolve(bodies_, constraints.normal, constraints.friction, fixed_dt);
    constexpr int solver_iterations = 10;
    contact_constraint_solver_.Solve(bodies_, constraints.normal, constraints.friction, solver_iterations);
  ...
  if (ball_) {
    ...
    for (const physics::Box& wall : walls_) {
      ...
      if (physics::collisions::ComputeContact(wall, *ball_, contact)) {
        physics::ContactConstraintSolver::GenerateFromContact(contact, bodies_, dt, constraints.normal,
                                                              constraints.friction, restitution, baumgarte);
      }
    }
    // Ball v Ground
    if (ground_) {
